...
void relay_init(void) {
    gpio_config_t io_conf = {
        ...
        .mode = GPIO_MODE_OUTPUT, // Reverted to pure OUTPUT for maximum drive strength
    ...
    // Explicitly force the hardware to match our software state variable
    // This prevents accidental power-cuts if relay_init() is ever called twice
    gpio_set_level(RELAY_1_PIN, r1_state ? 1 : 0);
    gpio_set_level(RELAY_2_PIN, r2_state ? 1 : 0);
    
    ESP_LOGI(TAG, "Relays initialized with pure OUTPUT mode. R1: %d, R2: %d", r1_state, r2_state);
}
